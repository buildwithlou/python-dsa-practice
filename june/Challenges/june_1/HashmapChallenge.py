...
for transaction in transactions:
    ...
    if user in count:
        count[user] = (
            count[user] + amount
        )  # if we have already seen this user, we add the amount to their total
    else:
        count[user] = (
            amount  # if we haven't seen this user before, we add them to the dictionary with their amount
        )
